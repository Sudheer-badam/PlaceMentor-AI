...
<div style='margin-top: 15px;'>
    <p id='live-clock' style='color: #00f2fe; font-family: "Courier New"; font-weight: bold; font-size: 0.85em; margin: 0; background: rgba(0,0,0,0.6); padding: 8px 15px; border-radius: 8px; display: inline-block; border: 1px solid rgba(0,242,254,0.4); box-shadow: 0 0 10px rgba(0,242,254,0.2);'>SYS TIME: INITIATING CLOCK...</p>
</div>
</div>
""", unsafe_allow_html=True)
        ...
        function updateTime() {
            ...
            if (clock) {
                ...
                m = m < 10 ? '0'+m : m;
                s = s < 10 ? '0'+s : s;
                clock.innerHTML = "SYS TIME: " + days[nd.getDay()] + ", " + (nd.getDate()<10?'0':'') + nd.getDate() + " " + months[nd.getMonth()] + " " + nd.getFullYear() + " | " + h + ':' + m + ':' + s + ' ' + ampm + " IST";
            }
        }
...
ALL FIELDS INCLUDING CAPTCHA ARE MANDATORY
</p>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
            ...
            with st.form("reg_form"):
                ...
                st.markdown("---")
                st.write("**SECURITY RECOVERY PROTOCOL**")
                ...
                if submit_reg:
                    with st.spinner("INITIALIZING IDENTITY PROTOCOLS..."):
...
""", unsafe_allow_html=True)
    ...
    col_nav1, col_nav2, col_nav3 = st.columns([1.2, 2.3, 1.5])
    with col_nav1:
        st.markdown("""
        <div class="landing-logo">
            <div class="landing-logo-icon">P</div>
            <div class="landing-logo-text">PlaceMentor AI</div>
        </div>
        """, unsafe_allow_html=True)
    with col_nav2:
        st.markdown("""
        <div class="landing-nav-links" style="display: flex; gap: 28px; justify-content: center; align-items: center; height: 100%;">
            <a href="#features" style="pointer-events: none;">Features</a>
            <a href="#predictor" style="pointer-events: none;">Predictor</a>
            <a href="#resume" style="pointer-events: none;">Resume Analyzer</a>
            <a href="#quiz" style="pointer-events: none;">Quiz System</a>
        </div>
        """, unsafe_allow_html=True)
    with col_nav3:
        col_nav_btn1, col_nav_btn2 = st.columns(2)
        with col_nav_btn1:
    ...
    if st.session_state.auth_view == "landing":
        # Render the Dribbble Hero Section
        col_hero_1, col_hero_2 = st.columns([1.1, 0.9], gap="large")
        with col_hero_1:
            # Hero Left
            st.markdown("""
            <div class="landing-hero-left" style="margin-top: 20px;">
                <h1 class="landing-hero-title">
                    <span class="gradient-text">AI analysis</span><br>
                    for career readiness.
                </h1>
                <p class="landing-hero-subtitle" style="margin-top: 15px; margin-bottom: 25px;">
                    PlaceMentor AI analyzes your resume, evaluates aptitude, and calculates real-time placement probability using a robust Random Forest classifier — all to secure your path to top companies.
                </p>
            </div>
            """, unsafe_allow_html=True)
            
            # CTAs
            col_cta_1, col_cta_2 = st.columns([1.2, 1])
            with col_cta_1:
                st.markdown('<div class="primary-btn-container">', unsafe_allow_html=True)
                if st.button("Start for free →", key="hero_start_btn"):
                    st.session_state.auth_view = "signup"
                    st.rerun()
                st.markdown('</div>', unsafe_allow_html=True)
            with col_cta_2:
                st.markdown('<div class="secondary-btn-container">', unsafe_allow_html=True)
                if st.button("Access Portal", key="hero_login_btn"):
                    st.session_state.auth_view = "login"
                    st.rerun()
                st.markdown('</div>', unsafe_allow_html=True)
            ...
            # Trust badges
            st.markdown("""
            <div class="landing-hero-badges" style="margin-top: 25px;">
                <span class="landing-badge-item">KLU Verified</span>
                <span class="landing-badge-item">No fees required</span>
                <span class="landing-badge-item">Cancel preparation anytime</span>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_hero_2:
            ...
            <div class="landing-hero-right">
                <!-- SVG Connector Lines -->
                <svg class="connector-svg" viewBox="0 0 400 400">
                    <path d="M 60 140 Q 150 160 150 200" fill="none" stroke="#8552D8" stroke-width="2" stroke-dasharray="4" opacity="0.6" />
                    <path d="M 60 310 Q 150 290 150 240" fill="none" stroke="#E87654" stroke-width="2" stroke-dasharray="4" opacity="0.6" />
                    <path d="M 340 140 Q 250 160 250 200" fill="none" stroke="#8552D8" stroke-width="2" stroke-dasharray="4" opacity="0.6" />
                    <path d="M 340 310 Q 250 290 250 240" fill="none" stroke="#E87654" stroke-width="2" stroke-dasharray="4" opacity="0.6" />
                </svg>

                <!-- Rating Badge -->
                <div class="g2-rating-badge">
                    <span class="stars">★ ★ ★ ★ ★</span>
                    <span>5.0 / 5.0 Rated</span>
                </div>

                <!-- Floating Feed 1 (Top Left) -->
                <div class="floating-feed-card feed-pos-1">
                    <img src="data:image/jpeg;base64,{profile_photo_data}" alt="Sudheer">
                    <div class="feed-name">Sudheer</div>
                </div>

                <!-- Floating Feed 2 (Bottom Left) -->
                <div class="floating-feed-card feed-pos-2">
                    <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150" alt="Sofia">
                    <div class="feed-name">Sofia</div>
                </div>

                <!-- Floating Feed 3 (Top Right) -->
                <div class="floating-feed-card feed-pos-3">
                    <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150" alt="Conrad">
                    <div class="feed-name">Conrad</div>
                </div>

                <!-- Floating Feed 4 (Bottom Right) -->
                <div class="floating-feed-card feed-pos-4">
                    <img src="https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=150" alt="Meera">
                    <div class="feed-name">Meera</div>
                </div>

                <!-- Central Mockup Phone -->
                <div class="mockup-main-phone">
                    <div class="phone-header">
                        <span class="phone-title" style="color: white !important;">Neural Predictor</span>
                        <span style="font-size: 0.6rem; color: #8A7A71; font-weight: bold;">PM-v4.0</span>
                    </div>
                    ...
                    <div class="phone-wave">
                        ...
                        <div class="wave-bar" style="height: 20px; animation-delay: 0.3s;"></div>
                        <div class="wave-bar" style="height: 45px; animation-delay: 0.2s;"></div>
                        ...
                        <div class="wave-bar" style="height: 25px; animation-delay: 0.6s;"></div>
                    </div>

                    <div class="phone-timer">
                        <span style="color: white !important;">98.6% Accuracy</span>
                        <span class="analysing">📊 LIVE</span>
                    </div>

                    <!-- Transcript Card 1 -->
                    <div class="phone-transcript-card">
                        <div class="transcript-user">
                            <div class="transcript-avatar" style="background-color: #8552D8;"></div>
                            <div class="transcript-name">Resume Analysis</div>
                        </div>
                        <div style="color: #EAE0D9 !important;">ATS score: 92/100. Key skills matched: Python, SQL, Random Forest. Ready for interview.</div>
                    </div>

                    <!-- Transcript Card 2 -->
                    <div class="phone-transcript-card" style="margin-bottom: 0;">
                        <div class="transcript-user">
                            <div class="transcript-avatar" style="background-color: #E87654;"></div>
                            <div class="transcript-name">Placement Prediction</div>
                        </div>
                        <div style="color: #EAE0D9 !important;">Odds: High Probability. Suggestion: Focus on SQL and System Design fundamentals.</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        col_form_1, col_form_2 = st.columns([1, 1.2], gap="large")
        with col_form_1:
            st.markdown(f"""
            <div class="dev-card-landing">
                <img src="data:image/jpeg;base64,{profile_photo_data}" class="dev-photo" alt="Developer photo">
                <h3 style="font-family: 'Orbitron', sans-serif !important; font-weight: 800; margin: 10px 0;">BADAM SUDHEER REDDY</h3>
                <p style="color: #4F4F4F; font-weight: bold; margin: 4px 0; font-size: 1.1em;">ID: 2300033278</p>
                <p style="color: #8A7A71; font-size: 0.95rem; line-height: 1.4;">Computer Science & Engineering<br>KL University</p>
                <hr style="border: none; border-top: 1px solid #EAE0D9; margin: 20px 0;">
                <div style="display: flex; justify-content: center; gap: 10px;">
                    <span style="background: #8552D8; color: white; padding: 6px 16px; border-radius: 20px; font-size: 0.75rem; font-weight: bold; box-shadow: 0 4px 10px rgba(133,82,216,0.25);">AI SPECIALIST</span>
                    <span style="background: #E87654; color: white; padding: 6px 16px; border-radius: 20px; font-size: 0.75rem; font-weight: bold; box-shadow: 0 4px 10px rgba(232,118,84,0.25);">FULL STACK</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_form_2:
            st.markdown("""
            <h3 style="font-family: 'Inter', sans-serif; font-weight: 800; color: #1A1A1A; margin-top: 0; margin-bottom: 20px;">System Access Gateway</h3>
            """, unsafe_allow_html=True)
            ...
            with st.form("login_form"):
                ...
                st.markdown(f"""
                <div style='text-align: center;'>
                    <p style='margin: 0; color: #8552D8; font-size: 0.75em; letter-spacing: 2px; font-weight: bold;'>VERIFICATION CODE</p>
                    <h1 style='letter-spacing: 15px; color: #1A1A1A; margin: 10px 0; font-family: "Courier New"; text-shadow: none;'>{st.session_state.captcha}</h1>
                </div>
                """, unsafe_allow_html=True)
            ...
            with st.expander("🔑 LOST ACCESS? INITIATE RECOVERY"):
                st.markdown("""
                <div style='background: rgba(232, 118, 84, 0.08); padding: 15px; border-radius: 10px; border-left: 4px solid #E87654; border: 1px solid #EAE0D9;'>
                    <p style='font-size: 0.8em; color: #E87654; margin: 0; font-weight: bold;'>RECOVERY PROTOCOL</p>
                    <p style='font-size: 0.75em; color: #4F4F4F; margin: 0;'>Provide your identifier to initiate security challenge.</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                if f_user:
                    ...
                    if q:
                        st.markdown(f"""
                        <div style='margin: 15px 0; padding: 10px; background: rgba(133, 82, 216, 0.08); border-radius: 8px; border: 1px dashed #8552D8;'>
                            <p style='font-size: 0.75em; color: #8552D8; margin-bottom: 5px;'>SECURITY CHALLENGE:</p>
                            <p style='font-weight: bold; color: #1A1A1A;'>{q}</p>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        with col_form_2:
            st.markdown("""
            <h3 style="font-family: 'Inter', sans-serif; font-weight: 800; color: #1A1A1A; margin-top: 0; margin-bottom: 20px;">Initialize Identity Node</h3>
            """, unsafe_allow_html=True)
            
            with st.form("reg_form"):
                new_uni_in = st.selectbox("YOUR UNIVERSITY / COLLEGE", COLLEGE_NAMES, key="reg_uni")
                new_u_in = st.text_input("QUANTUM NAME (USERNAME)", key="reg_user")
                new_e_in = st.text_input("IDENTITY EMAIL", key="reg_email")
                new_p_in = st.text_input("ACCESS KEY (PASSWORD)", key="reg_pass", type="password")
                new_c_in = st.text_input("CONFIRM KEY", key="reg_confirm", type="password")
    ...
    # Global Promo & Logo Cloud at the bottom of the card
    st.markdown("<div style='margin-top: 60px;'></div>", unsafe_allow_html=True)
    
    st.markdown("""
    <div class="landing-promo-bar">
        Enjoy 100% free placement preparation for the first 3 months — 21 days remaining 
        <a href="#signup" style="pointer-events: none;">Start 14 days trial</a>
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown("""
    <div class="landing-logos-cloud">
        <div class="partner-logo">🏫 KL UNIVERSITY</div>
        <div class="partner-logo">🚀 PlaceMentor AI</div>
        <div class="partner-logo">🛒 Sudheer Mart</div>
        <div class="partner-logo">🤖 Google AI</div>
        <div class="partner-logo">💻 Microsoft</div>
    </div>
    """, unsafe_allow_html=True)

    # News Ticker (render below if not logged in)
